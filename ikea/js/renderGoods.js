...
const renderGoods = () => {
  ...
  const renderGoodsCards = (data) => {
    ...
    data.forEach(({ id, img, name, price, description, count }) => {
      goodsList.insertAdjacentHTML(
        ...
        `	<li class="goods-list__item">
              ...
              ${!count ? '<p class="goods-item__new">Нeт в наличии</p>' : ""}
  ...
  if (location.pathname.includes("goods") && location.search) {
    ...
    if (prop === "s") {
      getData.search(value, renderGoodsCards);
      mainHeader.textContent = `Поиск ${value}`;
    } else if (prop === "wishlist") {
      getData.wishList(userData.wishList, renderGoodsCards);
      mainHeader.textContent = `Список желаний`;
    } else if (prop === "cat" || prop === "subcat") {
      getData.category(prop, value, renderGoodsCards);
